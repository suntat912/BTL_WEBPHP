...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['contact_id'])) {
    ...
    if (!empty($feedback_message)) {
        // Lưu phản hồi vào bảng Feedback
...
if (isset($_GET['delete_id'])) {
    ...
    $_SESSION['success'] = "Liên hệ đã được xóa thành công!";
    header("Location: manage_contacts.php");
    exit();
}
?>
    ...
    <script>
        ...
        function confirmDelete(id) {
            if (confirm("Bạn có chắc chắn muốn xóa liên hệ này?")) {
                window.location.href = "manage_contacts.php?delete_id=" + id;
            }
        }

        // Hiện form phản hồi cho liên hệ đã chọn
        function showFeedbackForm(contactId) {
...
<body>
    ...
    <table>
        ...
        <?php foreach ($contacts as $contact): ?>
            <tr>
                <td><?php echo htmlspecialchars($contact['full_name'] ?? 'N/A'); ?></td>
                <td><?php echo htmlspecialchars($contact['email'] ?? 'N/A'); ?></td>
                <td><?php echo htmlspecialchars($contact['message'] ?? 'N/A'); ?></td>
                <td><?php echo htmlspecialchars($contact['created_at'] ?? 'N/A'); ?></td>
                <td>
                    <a href="javascript:void(0);" onclick="showFeedbackForm(<?php echo $contact['id']; ?>)" class="button">
                        <i class="fas fa-reply"></i> Phản hồi
                    </a>
                    <div class="divider"></div> <!-- Vách ngăn -->
    ...
    <!-- Form phản hồi -->
    <div id="feedback-modal" class="feedback-form">
        <h3>Gửi Phản Hồi</h3>
